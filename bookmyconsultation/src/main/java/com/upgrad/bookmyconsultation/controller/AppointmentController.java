...
public class AppointmentController {
    ...
    public ResponseEntity bookAppointment(@RequestBody Appointment newAppointment) throws SlotUnavailableException, InvalidInputException {
        String savedAppointmentId =
                appointmentService.appointment(newAppointment);
        return new ResponseEntity(savedAppointmentId, HttpStatus.CREATED);
    }
    ...
    @GetMapping(value = "/appointmentId", consumes =
            MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity getAppointment(@PathVariable(name = "appointmentId") String appointmentId) {
        Appointment fetchedAppointment =
                appointmentService.getAppointment(appointmentId);
        return new ResponseEntity<>(fetchedAppointment, HttpStatus.OK);
    }
