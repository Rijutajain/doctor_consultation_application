...
public class RatingsService {
    ...
    public void submitRatings(Rating rating) {
        ...
        String docId = savedRating.getDoctorId();
        List<Rating> RatingsOfDoc = ratingsRepository.findByDoctorId(docId);
        int totalRating = 0;
        for (Rating r : RatingsOfDoc) {
            totalRating = totalRating + r.getRating();
        }
        double avgRating = totalRating / RatingsOfDoc.size();
        Doctor doc = doctorRepository.findById(docId).get();
        doc.setRating(avgRating);
